...
- **Solicitar Costo – Asignación grupal automática**
  ...
  - **UI "Equipo Compras / Jefes Comerciales"**: En las tarjetas de cotización, cuando no hay un asignado individual, se reemplaza el texto `Asignado -` por `Equipo Compras / Jefes Comerciales` para indicar que es una tarea grupal.
  - **Workspace Compras**: El listado de cotizaciones en la pestaña `Compras` ahora muestra (a) cotizaciones que el usuario tiene asignadas individualmente Y (b) cotizaciones sin asignación individual (pendientes de todo el equipo).
...
- **Backup y Restauración – Fix de Foreign Keys durante el restore**
  ...
  - **Impacto**: el fix cubre no solo `cotizacion_comentarios → cotizaciones`, sino cualquier relación del esquema (ej: `informe_tecnico_comentarios → informes_tecnicos`, `cotizacion_documentos → cotizaciones`, `proyecto_documentos → proyectos`, `registros → tecnicos/clientes/tipos_tarea`, etc.), evitando que futuras tablas nuevas rompan el restore por cambios en el orden de inserción.
