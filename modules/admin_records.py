...
def render_records_management(df, role_id=None):
        ...
        if st.button("🚀 Procesar y Cargar Datos", key=f"process_excel_{role_id if role_id else 'default'}"):
            with st.spinner("Procesando archivo Excel..."):
                try:
                    ...
                    # AGREGAR: Asignación automática de registros después del procesamiento
                    ...
                    # Mostrar resumen consolidado de resultados
                    ...
                    # NUEVO: Marcar procesamiento exitoso para eliminar archivo en el siguiente render
    ...
    # Mostrar tabla de registros existente
    ...
    with st.expander("Editar o Eliminar Registro (Admin)"):
        if not df.empty:
            ...
            if selected_registro_admin:
                ...
                with col3:
                    st.info(f"**Modalidad:** {registro_seleccionado_admin['modalidad']}\n**Tiempo:** {registro_seleccionado_admin['tiempo']}h")
...
def render_admin_edit_form(registro_seleccionado, registro_id, role_id=None):
    ...
    # Obtener datos para los selectboxes
    ...
    with st.form(key=f"edit_form_admin_{registro_id}_{role_id if role_id else 'default'}"):
        ...
        # Tarea realizada
        tarea_actual = registro_seleccionado['tarea_realizada']
        nueva_tarea = st.text_area("Tarea Realizada", value=tarea_actual, height=100)
        
        # Número de ticket
        ticket_actual = registro_seleccionado.get('numero_ticket', 'N/A')
        nuevo_ticket = st.text_input("Número de Ticket", value=ticket_actual)
        
        # Descripción
        descripcion_actual = registro_seleccionado.get('descripcion', '')
        nueva_descripcion = st.text_area("Descripción", value=descripcion_actual, height=80)
        ...
        if submitted:
            if not nueva_tarea:
                ...
                try:
                    ...
                    if not check_record_duplicate(fecha_formateada, id_tecnico_admin, id_cliente_admin, 
...
def render_admin_delete_form(registro_seleccionado, registro_id, role_id=None):
    ...
    # Mostrar detalles del registro a eliminar
    st.info(f"""
    **Registro a eliminar:**
    - **ID:** {registro_id}
